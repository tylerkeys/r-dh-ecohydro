...
elf_qr_plot <- function(inputs,qr_calc_out, data, startdate, enddate){
  ...
  #Load inputs from qr_calc_out
  rucount <- qr_calc_out$qr_stats_out$stat_quantreg_n
  yaxis_thresh <- qr_calc_out$qr_stats_out$stat_quantreg_max
  upper.quant <- qr_calc_out$qr_upper.quant
  #Display only 3 significant digits on plots
  plot_ruslope <- signif(qr_calc_out$qr_stats_out$stat_quantreg_m, digits = 3)
  plot_ruint <- signif(qr_calc_out$qr_stats_out$stat_quantreg_b, digits = 3)
  plot_rurs <- signif(qr_calc_out$qr_stats_out$stat_quantreg_rsq, digits = 3)
  ...
  subset_n <- length(data$metric_value)
        ...
        #Plot titles
        plot_title <- paste(target_hydrocode,"\n\nQuantile Regression: (breakpoint at DA = ", ghi, #removed the start and enddates
