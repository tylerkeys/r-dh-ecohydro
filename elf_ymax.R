...
elf_ymax <- function(inputs, data, x_metric_code, Feature.Name_code, Hydroid_code, search_code, token){
  ...
  sampres <- inputs$sampres
        ...
        x.ymax <- subset(dme, dme$metric_value == ymax)
        breakpt <- min(x.ymax$attribute_value)
        
        #use breakpt to split dme into two parts
        data1 <- subset(dme, attribute_value > 0.001 & attribute_value < breakpt)
        data2 <- subset(dme, attribute_value >= breakpt)
...
print (paste("Plotting Barplot"));
            #slope barplot  
              ...
              print(paste("... Skipping (fewer than 10 datapoints to the right of x-axis inflection point in ", search_code,")", sep=''));
              }
              
            } else {
              print(paste("... Skipping (fewer than 10 datapoints to the left of x-axis breakpoint in ", search_code,")", sep=''));
            }
